...
def _row_label_map(rows: list[tuple[object, ...]]) -> dict[str, tuple[object, ...]]:
...
def parse_archon_xlsx(data: bytes) -> TdaEventMeta:
    ...
    rounds_format = _normalize_rounds(int(info["Number of Rounds (including final):"][1]))
    players_count = int(info["Number of Players:"][1])
...
def _seat_to_vekn_map(rows: list[tuple[object, ...]]) -> dict[int, int]:
...
def _winner_from_standings(rows: list[tuple[object, ...]]) -> tuple[str, int | None]:
